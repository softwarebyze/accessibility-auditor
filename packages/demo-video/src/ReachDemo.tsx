...
const Intro: React.FC = () => {
  ...
  const subDelay = 12;
  const subSpring = spring({ frame: frame - subDelay, fps, config: { damping: 200 } });
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ fontSize: 88, fontWeight: 700, color: theme.text, letterSpacing: '-0.03em' }}>
          Reach
        </div>
        <div
          style={{
...
const FeaturesScene: React.FC = () => {
  ...
  return (
    ...
    >
      <SceneHeader eyebrow="Everything included" title="One tool, full coverage" />
      <div
        style={{
          display: 'grid',
      ...
      >
        {features.map((f, i) => {
          ...
          return (
            ...
            >
              <div style={{ color: theme.accent, fontSize: 22, fontWeight: 700 }}>{f.label}</div>
              <div style={{ color: theme.muted, fontSize: 16, marginTop: 6 }}>{f.desc}</div>
            </div>
          );
        })}
      </div>
...
const Outro: React.FC = () => {
  ...
  const pulse = interpolate(frame % (fps * 2), [0, fps, fps * 2], [1, 1.02, 1], {
  ...
  return (
    ...
    >
      <div style={{ opacity: interpolate(s, [0, 1], [0, 1]), transform: `scale(${pulse})` }}>
        <div style={{ fontSize: 36, fontWeight: 700, color: theme.text }}>Try it now</div>
        <div
          style={{
            marginTop: 24,
...
export const REACH_DEMO_FRAMES = 3 * FPS + 8 * FPS + 14 * FPS + 10 * FPS + 8 * FPS + 7 * FPS + 5 * FPS;
