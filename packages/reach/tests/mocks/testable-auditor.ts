...
export class TestableAccessibilityAuditor {
  ...
  async audit(url: string, options: TestAuditOptions = {}): Promise<AuditResult> {
    ...
    try {
      ...
      const violations = this.processViolations(axeResults.violations as AxeViolation[]);
      ...
      return {
        ...
        violations,
        rawAxeResults: axeResults as unknown as import('../../src/core/types.js').AxeResults,
      };
    } finally {
      await page.close();
      await context.close();
   ...
   */
  private async setPageContent(page: Page, html: string, _baseUrl: string): Promise<void> {
