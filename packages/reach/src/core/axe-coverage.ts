...
function getCoverageStats() {
  const verified = AXE_COVERAGE_ANALYSIS.checks.filter((c) => c.verified).length;
...
export function getCoverageSummary(): string {
  ...
  summary += '   💡 Automated testing catches ~30-50% of accessibility issues\n';
...
export function getCoverageOverview(): string {
  ...
  overview += '💡 Automated testing typically covers ~30-50% of accessibility issues\n';
...
export function getCoverageReport(): string {
  ...
  for (const check of AXE_COVERAGE_ANALYSIS.checks.filter((c) => !c.verified).slice(0, 10)) {
    // Show first 10
    const impactIcon = getImpactIcon(check.impact);
    report += `${impactIcon} ${check.id}: ${check.description}\n`;
  }
  ...
  report += '   Axe-core is a mature, widely-used accessibility testing engine\n';
  ...
  report += '   • Some issues require human judgment (e.g., meaningful alt text)\n';
  report += '   • Dynamic content and complex interactions need manual testing\n';
