...
import { getCoverageOverview, getCoverageSummary } from '../core/axe-coverage.js';
import type { AuditResult, AuditSummary, ManualCheckResult, Violation } from '../core/types.js';

export interface ConsoleReporterOptions {
  verbose?: boolean;
}

export function report(result: AuditResult, options: ConsoleReporterOptions = {}): void {
  console.log(`\n${chalk.bold.blue('🔍 Reach — Accessibility Audit Report')}`);
...
function printSummary(summary: AuditSummary): void {
  ...
  if (totalViolations === 0) {
    ...
    if (criticalViolations > 0) console.log(chalk.red(`    🔴 Critical: ${criticalViolations}`));
    if (seriousViolations > 0) console.log(chalk.yellow(`    🟡 Serious: ${seriousViolations}`));
    if (moderateViolations > 0) console.log(chalk.blue(`    🔵 Moderate: ${moderateViolations}`));
    if (minorViolations > 0) console.log(chalk.gray(`    ⚪ Minor: ${minorViolations}`));
  }
...
function printViolations(violations: Violation[]): void {
  ...
  violations.forEach((violation, index) => {
    ...
    console.log(chalk.bold(`${index + 1}. ${impactIcon} ${violation.description}`));
    console.log(chalk.gray(`   Impact: ${impactColor(violation.impact.toUpperCase())}`));
    console.log(chalk.gray(`   WCAG Level: ${violation.wcagLevel}`));
    console.log(chalk.blue(`   Help: ${violation.helpUrl}`));

    if (violation.nodes.length > 0) {
      console.log(chalk.gray(`   Affected elements: ${violation.nodes.length}`));
      violation.nodes.slice(0, 3).forEach((node, nodeIndex) => {
        console.log(chalk.gray(`     ${nodeIndex + 1}. ${node.target.join(' ')}`));
      });
      if (violation.nodes.length > 3) {
        console.log(chalk.gray(`     ... and ${violation.nodes.length - 3} more`));
      }
    }
      ...
      console.log(
        chalk.gray(
          `     • Automation: ${check.automation.toUpperCase()}${
            check.relatedRuleIds?.length ? ` (rules: ${check.relatedRuleIds.join(', ')})` : ''
          }`
        )
      );
...
function printLegalNote(): void {
  ...
  console.log(chalk.yellow('   This automated scan catches ~30-50% of accessibility issues.'));
