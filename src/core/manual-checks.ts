...
export const MANUAL_CHECK_DEFINITIONS: ManualCheckDefinition[] = [
  {
    ...
    manualNotes:
      "Review alt text manually to ensure it communicates intent and context.",
  },
  {
    id: "page-title",
    title: "Page Title",
  ...
  },
  {
    ...
    category: "common",
    description:
      "Validates heading structure and hierarchy so content is navigable by outline.",
    whyItMatters:
  ...
  },
  {
    ...
    category: "common",
    description:
      "Measures contrast between text or interactive elements and their backgrounds.",
    whyItMatters:
  ...
  },
  {
    ...
    category: "common",
    description:
      "Checks for keyboard-only mechanisms to bypass repeated navigation.",
    whyItMatters:
    ...
    manualNotes:
      "Confirm the skip link is visible on focus and targets the correct region.",
  },
  {
    id: "visible-focus",
  ...
  },
  {
    ...
    description: "Ensures media with audio includes synchronized captions covering speech and relevant sounds.",
    whyItMatters:
      "Captions make audio content accessible to Deaf and hard-of-hearing audiences.",
    whatToLookFor: [
  ...
  },
  {
    ...
    title: "Transcripts",
    category: "audiovisual",
    description: "Confirms audio and video assets provide full text transcripts separate from the player.",
    whyItMatters:
  ...
  },
  {
    ...
    category: "forms",
    description:
      "Validates that every form field exposes a programmatic and visible label.",
    whyItMatters:
  ...
  },
  {
    ...
    whyItMatters:
      "People need advance notice of required information to avoid validation errors.",
    whatToLookFor: [
...
const CATEGORY_ORDER: ManualCheckDefinition["category"][] = [
  "common",
  "audiovisual",
  "forms",
];

export function buildManualCheckResults(
  ...
  return MANUAL_CHECK_DEFINITIONS.map((definition) => {
    ...
    const hasRelatedViolations = related.some((id: string) =>
      violationIds.has(id)
    );
    const hasRelatedPasses = related.some((id: string) => passIds.has(id));
    const hasRelatedIncomplete = related.some((id: string) =>
    ...
    if (definition.automation === "manual") {
      ...
      status = "needs-review";
      notes =
        definition.manualNotes ??
        "Automated engine marked this rule incomplete; review manually.";
